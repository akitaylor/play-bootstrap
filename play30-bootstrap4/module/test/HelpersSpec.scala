...
class HelpersSpec extends Specification {
  ...
  "@hidden" should {
    ...
    "with filled Field object" in {
      val body =
        clean(b4.hidden(fooFieldFilled("filledValue"), Symbol("value") -> "testValue", Symbol("foo") -> "bar").body)
  ...
  "@formGroup" should {
    ...
    "vertical: show label" in {
      testFormGroup(Symbol("_class") -> "theClass", Symbol("_id") -> "theId", Symbol("_label") -> "theLabel")(
        vfc,
        msgsProv
      ) must be equalTo clean("""
    ...
    "get the inner arguments for the content" in {
      val body = b4
        .freeFormGroup(
          Seq(Symbol("_class") -> "theClass", Symbol("_underscored") -> "underscored", Symbol("foo") -> "foo")
        )(innerArgsMap =>
          Html(innerArgsMap.toSeq.map(a => s"""${a._1.name}="${a._2.toString}"""").mkString("<content ", " ", ">"))
        )(vfc, msgsProv)
        .body
  ...
  "@free" should {
    "be rendered correctly" in {
      clean(b4.free(Symbol("foo") -> "fooValue")(Html("<content>"))(vfc, msgsProv).body) must be equalTo clean(
        b4.freeFormGroup(Seq(Symbol("foo") -> "fooValue"))(_ => Html("<content>"))(vfc, msgsProv).body
      )
    }
  }
  ...
  "@static" should {
    ...
    "render with form-control-static class as default" in {
      b4.static("theLabel")(Html("theText"))(vfc, msgsProv).body must contain(
        "<p class=\"form-control-static\">theText</p>"
      )
    }

    "allow setting additional classes" in {
      b4.static("theLabel", Symbol("class") -> "extra_class")(Html("theText"))(vfc, msgsProv).body must contain(
        "<p class=\"form-control-static extra_class\">theText</p>"
      )
    }
  ...
  "@buttonType" should {
    ...
    def buttonTypeBody(args: (Symbol, Any)*) =
      b4.buttonType(sampleType, args: _*)(Html(sampleContent))(vfc, msgsProv).body
    ...
    "be rendered correctly" in {
      ...
      body must contain(
        "<button type=\"" + sampleType + "\" id=\"someid\" class=\"btn btn-default\">" + sampleContent + "</button>"
      )
    }
  }

  def sampleButtonTypeBody(theType: String) =
    b4.buttonType(theType, sampleArgs: _*)(Html("content"))(vfc, msgsProv).body.trim
  ...
  "@inputWrapped" should {
    ...
    "wrap the input" in {
      ...
      def wrap(input: Html)         =
        HtmlFormat.fill(scala.collection.immutable.Seq(Html(wrapperPre), input, Html(wrapperPost)))
      ...
      body.substring(indexOfWrapperPre + wrapperPre.length, indexOfWrapperPost) must be equalTo bodyInputType.substring(
        indexOfWrapperPre,
        indexOfWrapperPost - wrapperPre.length
      )
      body.substring(indexOfWrapperPost, indexOfWrapperPost + wrapperPost.length) must be equalTo wrapperPost
      body.substring(indexOfWrapperPost + wrapperPost.length) must be equalTo bodyInputType.substring(
        indexOfWrapperPost - wrapperPre.length
      )
    }
  }
  ...
  "@multifield" should {
    ...
    def multifield(
      ...
      globalArgs: Seq[(Symbol, Any)] = Seq(),
      fieldsArgs: Seq[(Symbol, Any)] = Seq()
    )(fc: b4.B4FieldConstructor, msgsProv: MessagesProvider) =
      clean(
        b4.multifield(form("foo"), form("bar"))(globalArgs, fieldsArgs)(cfc => Html(testInputsString))(fc, msgsProv)
          .body
      )
    def fooMultifield(globalArgs: (Symbol, Any)*)              = multifield(fooForm, globalArgs)(vfc, msgsProv)
    def fooMultifieldWithFielsArgs(fieldsArgs: (Symbol, Any)*) =
      multifield(fooForm, fieldsArgs = fieldsArgs)(vfc, msgsProv)
    ...
    "allow setting extra classes form-group" in {
      fooMultifield(Symbol("_class") -> "extra_class another_class") must contain(
        "class=\"form-group extra_class another_class"
      )
    }

    "show label" in {
      multifield(fooForm, Seq(Symbol("_label") -> "fooLabel"))(vfc, msgsProv) must contain("<label>fooLabel</label>")
      multifield(fooForm, Seq(Symbol("_label") -> "fooLabel"))(hfc, msgsProv) must contain(
        "<label class=\"col-form-label " + colLabel + "\">fooLabel</label>"
      )
    }
    ...
    "allow showing constraints" in {
      fooMultifield(Symbol("_showConstraints") -> true) must contain(
        "<small class=\"form-text text-muted\">" + msgsProv.messages("constraint.required") + "</small>"
      )
    }

    "allow showing help info" in {
      fooMultifield(Symbol("_help") -> "test-help") must contain(
        """<small class="form-text text-muted">test-help</small>"""
      )
      fooMultifield(Symbol("_success") -> "test-help") must contain("""<div class="valid-feedback">test-help</div>""")
      fooMultifieldWithFielsArgs(Symbol("_success") -> "test-help") must contain(
        """<div class="valid-feedback">test-help</div>"""
      )
      fooMultifield(Symbol("_warning") -> "test-help") must contain("""<div class="warning-feedback">test-help</div>""")
      fooMultifieldWithFielsArgs(Symbol("_warning") -> "test-help") must contain(
        """<div class="warning-feedback">test-help</div>"""
      )
      fooMultifield(Symbol("_error") -> "test-help") must contain("""<div class="invalid-feedback">test-help</div>""")
      fooMultifieldWithFielsArgs(Symbol("_error") -> "test-help") must contain(
        """<div class="invalid-feedback">test-help</div>"""
      )
    }
    ...
    "render validation states" in {
      ...
      def testStatus(status: String, withFieldsArgs: Boolean, args: (Symbol, Any)*) = {
        val test =
          if (withFieldsArgs)
            clean(
              b4.multifield(fooForm("foo"))(globalArgs = Seq(), fieldsArgs = args)(cfc =>
                b4.text(fooForm("foo"), args: _*)
              )(vfc, msgsProv)
                .body
            )
          else
            clean(
              b4.multifield(fooForm("foo"))(globalArgs = args, fieldsArgs = Seq())(cfc =>
                b4.text(fooForm("foo"), args: _*)
              )(vfc, msgsProv)
                .body
            )
